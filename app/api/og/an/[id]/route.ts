...
async function fetchArticle(id: string) {
  const url = `${SUPA_URL}/rest/v1/site_articles?id=eq.${id}&published=eq.true&select=title,og_image,cover_image,images`
...
export async function GET(_: Request, { params }: { params: { id: string } }) {
  const row = await fetchArticle(params.id)
  ...
  return new ImageResponse(
    (
      ...
      >
        <div style={{ fontSize: 28, opacity: 0.9 }}>Munchies Art Club — Article</div>
